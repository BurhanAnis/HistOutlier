...
def process_npz(npz_path, out_dir, nu=0.01, max_normals = 10000, random_state = 42):
    ...
    print(f"Processing OCSVM in {out_dir}:")
    print(f"  Normal latents: {latents_normal.shape}, Test latents: {latents_test.shape}")

    # Subsample normals if too large
    n_norm = latents_normal.shape[0]
    if n_norm > max_normals:
        np.random.seed(random_state)
        sel_idx = np.random.choice(n_norm, max_normals, replace=False)
        latents_normal = latents_normal[sel_idx]
        print(f"  Subsampled normal latents to {latents_normal.shape}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Train conv-autoencoder on WSI patches stored in LMDB and optionally evaluate with OCSVM"
    )
    # Autoencoder args
    ...
    # OCSVM args
    parser.add_argument('--run_ocsvm', action='store_true',
                        help="After AE, run One-Class SVM evaluation on saved latents")
    ...
    # ---------- Autoencoder Training & Latent Extraction ----------
    ...
    # ---------- Optional OCSVM Evaluation ----------
    if args.run_ocsvm:
        if not os.path.exists(cache_file):
            raise FileNotFoundError(f"Cache file not found: {cache_file}")
        process_npz(cache_file, args.output_dir, nu=args.nu)
